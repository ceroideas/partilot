...
<head>
    ...
    <title>Devolución a administración - Partilot</title>
    <style>
...
<div class="container">
    <div class="header">
        <h1>Devolución recibida - Partilot</h1>
    </div>

    @php
        $managerName = trim(($adminManagerUser?->name ?? '') . ' ' . ($adminManagerUser?->last_name ?? ''));
        $managerName = $managerName !== '' ? $managerName : ($adminManagerUser?->email ?? 'Gestor');
